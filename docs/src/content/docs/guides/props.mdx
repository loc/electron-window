...
:::note[`open={false}` and unload listeners]
Setting `open={false}` destroys the window via `BrowserWindow.destroy()`, which does **not** fire `beforeunload`/`unload`. If you attach those listeners via `useWindowDocument()`, they'll fire on user-initiated close (X button) but not on programmatic close.
:::
...
On macOS, `visibleOnAllWorkspaces` is applied with collection behaviors only (`skipTransformProcessType`): the window joins every Space and your app's own fullscreen Spaces, and the app keeps its Dock icon and Cmd+Tab entry. macOS does not let a regular (Dock-visible) app float a window over *another* app's fullscreen Space; if you need that, hide the Dock icon at the app level (`app.dock.hide()`) rather than relying on this prop.
