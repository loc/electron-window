...
This additionally gates the **preload**: on a wrong origin, `window.electron_window` is never exposed. The check runs in the EIPC validator layer against `event.senderFrame.url` (the correct frame).
...
| IPC from iframes                    | EIPC-generated `is_main_frame` validator — rejected before handlers run         |
