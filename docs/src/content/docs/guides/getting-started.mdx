...
The same applies to `window.open()` calls from **inside** a managed child window (a `<a target="_blank">` link in a popout). The library installs its own `setWindowOpenHandler` on every child — by default it denies everything. Pass `childWindowOpenHandler` to handle these, and `onChildWindowCreated` for anything else you need to attach (context menu, devtools):

```ts
manager.setupForWindow(mainWindow, {
  childWindowOpenHandler: ({ url }) => {
    if (url.startsWith("https://")) shell.openExternal(url);
    return { action: "deny" };
  },
  onChildWindowCreated: (win) => {
    win.webContents.on("context-menu", (_e, params) => showSpellcheckMenu(win, params));
  },
});
```

Don't try to do this from your own `webContents.on("did-create-window")` listener — that is the same event the library uses to install its handler, and which one wins depends on registration order and event-loop timing. `childWindowOpenHandler` and `onChildWindowCreated` run **after** the library has wired the child, so there's no race.

## Owner reload reaps children

When the owner WebContents performs a **hard main-frame navigation** (reload, `loadURL()`, full page nav), the library destroys that owner's managed child windows. Without this, a reload orphans the popouts: they stay on screen with stale content while the React tree that was portaling into them is gone.

The reaper does **not** fire on SPA `pushState` navigations or iframe loads — only hard navs. If your owner page re-adopts existing popouts after a reload, opt out:

```ts
manager.setupForWindow(mainWindow, { reapOnOwnerNavigate: false });
```
...
// O(1) "is this BrowserWindow one of yours?" — handy in app.on("browser-window-focus") etc.
app.on("browser-window-focus", (_e, win) => {
  if (manager.owns(win)) markPopoutActive(win);
});
```
...
`hideOnClose` pool windows veto their own `close` event. `app.quit()` waits for all `BrowserWindow`s to drain — a window that vetoes its own close stalls the quit forever. The library disarms its windows on `app.on("before-quit")` automatically (and re-arms them via `cancelQuit()` if another listener vetoes the quit), so plain `app.quit()`, the macOS `Cmd-Q` path, and unsaved-changes-prompt-then-cancel all Just Work. On Windows, `session-end` (logoff/shutdown) is handled per-window inside the library too.

The one gap: `electron-updater`'s `autoUpdater.quitAndInstall()` emits `before-quit-for-update` on **its own emitter**, not Electron's `app`. Call `prepareForQuit()` yourself there:
